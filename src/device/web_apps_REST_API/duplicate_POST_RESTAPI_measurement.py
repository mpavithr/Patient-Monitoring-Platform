...
if __name__ == "__main__":
    ...
    for i in range(len(measurement_data)):
        response = requests.post(BASE + "measurement/" + str(i), measurement_data[i])
        print(response.json())
    input()
    response = requests.post(BASE + "measurement/1",{"device_id": 2, "patient_id":3, "assigner_id":4, "machineID":123, "date_assigned":"123.34.001","date_returned":1, "measurement":32.0})
    print(response.json())
    '''
