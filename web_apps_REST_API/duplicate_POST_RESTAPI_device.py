...
if __name__ == "__main__":
    ...
    for i in range(len(device_data)):
        ...
        print(response.json())
    input()
    response = requests.post(BASE + "device/1",{"name": "Thermometer", "firmware_version":194.0, "date_of_purchase":"T234", "serial_number":123, "mac_address":"123.34.001","machineID":1})
    print(response.json())
    '''
    for i in range(len(device_data)):
            response = requests.post(BASE + "device/" + str(i), device_data[i])
